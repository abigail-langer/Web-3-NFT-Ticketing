...
const SellModal = ({ ticket, onClose, onSuccess }: SellModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('http://localhost:3001/api/market/sell', {
        method: 'POST',
        headers: {
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.error || 'Failed to list ticket.');
      }
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50">
      <div className="bg-gray-800 rounded-lg p-8 shadow-2xl w-full max-w-md">
        <h2 className="text-2xl font-bold mb-4">Sell Ticket #{ticket.id}</h2>
        <form onSubmit={handleSubmit}>
          <div className="mb-4">
            <label htmlFor="price" className="block text-sm font-medium text-gray-300 mb-2">
              Set Your Price (in ETH/USD, etc.)
            </label>
            <input
              type="number"
              id="price"
